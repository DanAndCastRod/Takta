...
**Version**: V2 (fases 1-6 cerradas al 2026-03-09)

Las instrucciones globales del agente estan en `~/.codex/AGENTS.md`.
El contrato Notion esta en el AGENTS.md del directorio padre (Ecosistema TD).
Este archivo agrega el contexto tecnico especifico de Takta.
...
#### Routers registrados (backend/app/main.py)

| Router               | Prefijo           | Dominio                              |
|----------------------|-------------------|--------------------------------------|
| auth                 | /api/auth         | Autenticacion y tokens               |
| assets               | /api/assets       | Arbol Sede->Linea->Maquina           |
...
| ci                   | /api/ci           | Mejora continua + KPI MC             |
...
| plant_layouts        | /api/plant-layouts| Layouts del Diagram Studio           |
...
- CSS: Tailwind 4 + tokens tk-* en src/style.css
...
| Servicio                  | Proposito                                              |
|---------------------------|--------------------------------------------------------|
| api.client.js             | Wrapper fetch con JWT y manejo de errores              |
| module-context.service.js | Contexto canonico global (asset_id, ref_id, std_id)    |
| ui-feedback.service.js    | Toasts -- unico mecanismo de feedback al usuario       |
...
- ImprovementAction / ActionWorkflow
- NonConformity / CapaAction
- ContinuousImprovementKpiDefinition / ContinuousImprovementKpiMeasurement
- WeightSample / WeightSpec / WeightCapabilityRun
- MeetingRecord / MeetingCommitment
- DiagramLayout / DiagramLibrary / DiagramPropertySchema
...
```powershell
# Backend
.\run_local.ps1                           # uvicorn :9003 hot-reload

# Frontend
cd frontend && npm run dev                # Vite :5173 proxy a :9003
...
- No romper definiciones de tabla SQLModel sin plan de migracion
- Guard de integridad: services/reference_guard.py para asset_id, product_reference_id, process_standard_id

### Frontend
- Nunca usar alert() -- toda notificacion via ui-feedback.service.js (toast)
- Componentes CSS con prefijo tk-* (tk-card, tk-input, tk-btn-secondary)
- Navegacion con hash routing (#/ruta) -- no usar window.location.href para rutas internas
- Contexto canonico gestionado exclusivamente por module-context.service.js
- Paginas con lazy-loading desde router.js -- mantener el patron al agregar rutas
- UTF-8 siempre

### General
- No proponer React, Next.js ni frameworks SPA
- No ejecutar builds ni tests para validar estado -- leer la documentacion
- Al agregar router: registrarlo en backend/app/main.py
- Al agregar pagina: registrarla en frontend/src/router.js y Sidebar.js
...
| URiT/EC&F        | Cloudflare Pages takta.urit.services | cPanel Python App (passenger_wsgi.py)     |
...
Procedimientos que el agente sigue cuando se invocan por nombre.

### status
1. Leer plans/v2/PLAN_MAESTRO_V2.md
2. Leer plans/v2/BITACORA_IMPLEMENTACION_V2.md
3. Leer archivos FASE_*.md relevantes
4. Producir tabla de fases, metricas y proximo bloque
5. NO ejecutar tests ni builds

### new-endpoint
1. Identificar router destino en backend/app/api/
2. Verificar modelo en models.py -- agregar si no existe
3. Escribir endpoint async con tipado completo
4. Agregar prueba en backend/tests/ siguiendo patron de conftest.py
5. Verificar registro en main.py
6. Actualizar task.md y walkthrough.md

### new-page
1. Crear frontend/src/pages/NombrePage.js siguiendo patron existente
2. Registrar ruta en frontend/src/router.js con import dinamico
3. Agregar entrada en frontend/src/components/layout/Sidebar.js
4. Usar uiFeedback para notificaciones, tk-* para componentes
5. Actualizar task.md y walkthrough.md

### debug
1. Pedir descripcion del sintoma
2. Leer el archivo relevante
3. Grep para buscar el sintoma en el codebase
4. Formular hipotesis antes de editar
5. Editar con cambio minimo necesario
6. Describir cambio y razon en walkthrough.md

### notion-sync
Seguir el contrato definido en el AGENTS.md del directorio padre (Ecosistema TD).
